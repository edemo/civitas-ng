...
public class CryptoBaseStub implements PrivateKeyTestData, PublicKeyTestData,
		...
		SharedKeyCiphertextTestData, PublicKeyCiphertextTestData {
	...
	public static CryptoBase stub()
		...
		when(mock.doCrypto(PUBLIC_KEY_CIPHER_ALG, PUBLIC_KEY_PROVIDER, PUBLIC_KEY,
				Cipher.ENCRYPT_MODE, SOMESTRING.getBytes()))
				.thenReturn(SOMESTRING_ENCRYPTED_BYTES);
		when(mock.doCrypto(PUBLIC_KEY_CIPHER_ALG, PUBLIC_KEY_PROVIDER, PRIVATE_KEY,
				Cipher.DECRYPT_MODE, SOMESTRING_ENCRYPTED_BYTES))
				.thenReturn(SOMESTRING.getBytes());
		when(mock.doCrypto(SHARED_KEY_CIPHER_ALG, SHARED_KEY_PROVIDER,
				SHARED_KEY_SPEC, Cipher.ENCRYPT_MODE, SOMESTRING.getBytes()))
				.thenReturn(SHARED_KEY_CIPHERTEXT_BYTES);
		when(mock.doCrypto(SHARED_KEY_CIPHER_ALG, SHARED_KEY_PROVIDER,
				SHARED_KEY_SPEC, Cipher.DECRYPT_MODE, SHARED_KEY_CIPHERTEXT_BYTES))
				.thenReturn(SOMESTRING.getBytes());
		...
		when(mock.getSharedKeyGenerator(SHARED_KEY_SIZE))
				.thenReturn(sharedGenerator);
