...
public class CryptoHashStub implements ElGamalProofDiscLogEqualityTestData,
	...
	public static CryptoHash stub() throws UnsupportedEncodingException {
		...
		when(mock.apply((TELLER_INDEX + VOTER_NAME).getBytes()))
				.thenReturn(VOTER_ADDITIONAL_ENV);
		when(mock.apply(TAB_TELLER_KEY_SHARE_XML.getBytes()))
		...
		when(mock.apply(PET_COMMITMENT_DI, PET_COMMITMENT_EI, null, null))
				.thenReturn(PET_COMMITMENT_HASH);
		when(mock.apply(EL_GAMAL_PROOF_KNOWN_DISC_LOG_V,
				EL_GAMAL_PROOF_KNOWN_DISC_LOG_A, null, null))
