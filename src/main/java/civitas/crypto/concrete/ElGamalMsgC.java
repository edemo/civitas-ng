...
/**
 * An element of an El Gamal message space. A <i>message space</i> is a
...
public class ElGamalMsgC implements ElGamalMsg {
	 ...
	 *                  plaintext is a value from some consecutive set of
	 *                  integers, e.g. Zq. This constructor encodes the plaintext
	 *                  as a message.
	...
	/**
	 * @throws CryptoException If s is empty or cannot be converted to a valid
	 *                         message in the message space defined by params.
	 */
	protected ElGamalMsgC(String s, ElGamalParametersC params)
			throws CryptoException {
		this(stringToBigInt(s), params);
	}

	public int plaintextIntValue(ElGamalParametersC params)
			throws NumberFormatException, CryptoException {
		return plaintextBigIntValue(params).intValue();
	}

	public CivitasBigInteger plaintextBigIntValue(ElGamalParametersC params)
			throws CryptoException {
		return params.decodeMessage(m);
	}

	public String plaintextStringValue(ElGamalParametersC params)
			throws CryptoException {
	...
	protected static CivitasBigInteger stringToBigInt(String s)
			throws CryptoException {
		if (s.length() < 1) {
			throw new CryptoException(
					"ElGamal messages cannot be constructed from empty strings");
		}
		try {
