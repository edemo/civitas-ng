...
class CryptoAlgs {
	...
	/**
	 * @return A random Schnorr prime p=2kq+1 where |q| = qLength and |p| =
	 *         pLength.
	 */
	...
	/**
	 * The number of attempts to generate a p from a given q. The DSA generation
	 * algorithm uses 4096, though no explanation for why is given. Some playing
	 * with probabilities and the prime number theorem reveals that, for
	 * pLength=1024, the probability of finding a prime is 2/711 in a single try,
	 * so the probability of finding a prime after 4096 tries is greater than 99%.
	 * Further analysis, based on treating this as a geometric process, suggests
	 * that, the number of tries needed for > 99% success is 2^(k+2) where k =
	 * log2(pLength). All this ignores the fact that we've already picked a given
	 * q, which all these probabilities should really be conditioned upon. It also
	 * ignores the fact that many tests get wasted because the rounding step in
	 * the DSA algorithm causes many p's to be rejected as the wrong size.
	 */
	...
	protected static CivitasBigInteger randomElement(CivitasBigInteger n) {
		...
		} while (r.equals(CivitasBigInteger.ZERO) || r.compareTo(n) >= 0); // while
																																				// r >=
																																				// n
	...
	/**
	 * @return A number that is probably prime and is length bits long, with false
	 *         positive probability <= 2^-CERTAINTY.
	 */
	...
	/**
	 * @return Whether n is a prime, with false positive probability <=
	 *         2^-CERTAINTY.
	 */
	 ...
	 * @return The Legendre symbol J_p(a): 1 if a \in QR_p, 0 if a mod p = 0, -1
	 *         otherwise.
	 */
	protected static int legendreSymbol(CivitasBigInteger a, CivitasBigInteger p,
			CivitasBigInteger q) {
