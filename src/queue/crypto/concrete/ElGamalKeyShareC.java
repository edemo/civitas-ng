...
class ElGamalKeyShareC extends ElGamalAbstractKeyShare {
    ...
    public void toXML(Label   PrintWriter s) {
        ...
        s.print("<pubKey>");
        if (pubKey != null) pubKey.toXML(  s);
        s.print("</pubKey>");
        s.print("<proof>");
        if (proof != null) proof.toXML(  s);
        s.print("</proof>");
        s.print("</elGamalKeyShare>");
    }

    public static ElGamalKeyShareC fromXML(Label   Reader r) throws IllegalArgumentException, IOException {
