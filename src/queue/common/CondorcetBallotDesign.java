...
public class CondorcetBallotDesign extends BallotDesign {
	...
	public VoterSubmission decompose(Ballot ballot, int voterBlock, ElGamalPublicKey key,
		...
		VerifiableVote[] votes = new VerifiableVote[matrixSize < 0 ? 0 : matrixSize];
		for (int i = 0; i < cb.k; i++) {
			for (int j = i + 1; j < cb.k; j++) {
				...
				try {
					...
					encChoice = CryptoUtil.factory().elGamal1OfLReencrypt(key, ciphertexts, 4, choice, encChoiceFactor);
				...
				try {
					...
					proofVote = CryptoUtil.factory().constructProofVote(key.getParams(), encCap, encChoice,
							desiredContext, encCapFactor, encChoiceFactor);
				} catch (NullPointerException imposs) {
				}
