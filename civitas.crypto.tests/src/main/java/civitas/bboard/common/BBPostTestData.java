...
public interface BBPostTestData
		extends ElectoralRollCapabilitySharesTestData,
				BoardClosedContentCommitmentTestData,
				SignatureTestData,
				CommonConstants {
	long SERIAL = 12L;
	...
	BBPost BBPOST = new BBPost(
			...
			SIGNATURE_ELECTORAL_ROLL_CAPABILITY_SHARES,
			BBPOST_HASH);
	BBPost BBPOST_BAD_SIG = new BBPost(
			BULLETIN_BOARD_ID,
			SERIAL,
			CURRENT_TIME,
			META_FOR_ELECTORAL_ROLL_CAPABILITY_SHARES,
			ELECTORAL_ROLL_CAPABILITY_SHARES_XML,
			SIGNATURE_ELECTORAL_ROLL_CAPABILITY_SHARES_BAD,
			BBPOST_HASH);
	BBPost NEXT_POST = new BBPost(
			BULLETIN_BOARD_ID,
			SERIAL + 1,
			CURRENT_TIME,
