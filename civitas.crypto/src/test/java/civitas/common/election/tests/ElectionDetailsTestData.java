...
import civitas.common.tests.UtilTestData;
...
public interface ElectionDetailsTestData
		extends ElectionIdTestData, PublicKeyTestData, BallotDesignTestData, ElGamalParametersTestData, UtilTestData {
	...
	ElectionDetails ELECTION_DETAILS = new ElectionDetails(
			ELECTION_ID.id(),
			PUBLIC_KEY2_BASE64,
			PUBLIC_KEY_BASE64,
			ELECTION_NAME,
			...
			STOP_TIME,
			FINALIZE_TIME,
			EL_GAMAL_PARAMETERS.p.i,
			EL_GAMAL_PARAMETERS.q.i,
			EL_GAMAL_PARAMETERS.g.i,
			KEYSIZE,
			NONCE_LENGTH,
			BLOCKSIZE);
}
