package civitas.common.mix.capabilitymixrevelation.tests;
...
public interface MixCapabilityElementRevelationTestData extends ElGamalReencryptFactorTestData {

	byte[] NONCE = BYTES;
	int MAPPING = 1;
	MixCapabilityElementRevelation CAPABILITY_ELEMENT_RELEVATION_MOCK = mock(MixCapabilityElementRevelation.class);
	MixCapabilityElementRevelation CAPABILITY_ELEMENT_RELEVATION =
			new MixCapabilityElementRevelation(1, NONCE, ELGAMAL_REENCRYPT_FACTOR_EPRIME);
	...
	byte[] CAPABILITY_ELEMENT_RELEVATION_RIGHT_HASH =
			Base64.getDecoder().decode("bRJm0nLPKMbXxhCoKbymzOUfYlK5T5ffjUNjhvm40C0=");

	byte[] CAPABILITY_ELEMENT_RELEVATION_LEFT_HASH =
			Base64.getDecoder().decode("nbuVKzKGSCyUQ3uS29JzAG22s3ngrPqeLNr/jMcJRDU=");
