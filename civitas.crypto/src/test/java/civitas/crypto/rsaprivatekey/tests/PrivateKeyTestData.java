package civitas.crypto.rsaprivatekey.tests;

import static org.mockito.Mockito.mock;

import java.security.spec.PKCS8EncodedKeySpec;
import java.util.Base64;

public interface PrivateKeyTestData {
	int KEYSIZE = 1024;
	String PRIVATE_KEY_FILE = "rsa_private_key.xml";

	String PRIVATE_KEY_BASE64 =
			"";
	String PRIVATE_KEY2_BASE64 =
			"";
	java.security.PrivateKey PRIVATE_KEY = mock(java.security.PrivateKey.class);
	java.security.PrivateKey PRIVATE_KEY2 = mock(java.security.PrivateKey.class);
	java.security.PrivateKey PRIVATE_KEY_BAD = mock(java.security.PrivateKey.class);
	String KEY_NAME = "alice";
	String KEY_NAME2 = "bob";

	PKCS8EncodedKeySpec KEYSPEC_PRIVATE =
			new PKCS8EncodedKeySpec(Base64.getDecoder().decode(PRIVATE_KEY_BASE64));
	PKCS8EncodedKeySpec KEYSPEC_PRIVATE2 =
			new PKCS8EncodedKeySpec(Base64.getDecoder().decode(PRIVATE_KEY2_BASE64));
	String PRIVATE_KEY_XML = "<PrivateKey><keyBase64>" + PRIVATE_KEY_BASE64 + "</keyBase64></PrivateKey>";
}
