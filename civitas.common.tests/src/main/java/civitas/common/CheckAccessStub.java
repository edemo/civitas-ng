...
public class CheckAccessStub implements BoardClosedContentCommitmentTestData,
	...
	public static CheckAccess stub() throws IllegalAccessException {
		...
		doNothing().when(mock).apply(Operation.POST, PUBLIC_KEY_BASE64,
				BoardClosedContentCommitmentMETA + BULLETIN_BOARD_ID);
		doNothing().when(mock).apply(Operation.POST, PUBLIC_KEY2_BASE64,
				BoardClosedContentCommitmentMETA + BULLETIN_BOARD_ID);
		return mock;
	}
}
