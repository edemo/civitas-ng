...
class AddVoteAndCommitmentToVoteMixTest extends RandomAwareTestBase implements VoteMixTestData, VoteTestData {
	...
	void test() {
		...
		verify(addVoteAndCommitmentToVoteMix.addCommitmentToMix).apply(VOTE_MIX_MOCK, BYTES);
		verify(addVoteAndCommitmentToVoteMix.addVoteToVoteMix).apply(VOTE_MIX_MOCK, VOTE_MOCK);
	}
