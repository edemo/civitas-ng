...
public class CheckVoterSubmission implements CommonConstants {
	...
	public void apply(
		...
		for (int i = 0; i < k; i++) {
			for (int j = i + 1; j < k; j++) {
				VerifiableVote vv = vs.votes[startIndex + calculatePositionInBallot.apply(i, j, k)];
				if (vv == null) {
				...
				if (!verifyVerifiableVote.apply(vv, pubKey, ciphertexts, VoteChoice.values().length + 1)) {
