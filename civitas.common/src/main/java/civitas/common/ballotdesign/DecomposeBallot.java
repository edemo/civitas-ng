...
public class DecomposeBallot implements CommonConstants {
	...
	public VoterSubmission apply(
			final BallotDesign that,
			final Ballot ballot,
			final int voterBlock,
		...
		for (int i = 0; i < ballot.k; i++) {
			for (int j = i + 1; j < ballot.k; j++) {
				...
				EncryptedChoice encryptedChoice = encryptChoice.apply(key, ciphertexts, cbMatrix, pos);

				String desiredContext = context + KIND + i + ":" + j;
				CapabilityEncryption encryptedCapability = encryptCapability.apply(key, capabilities, desiredContext);
				ProofVote proofVote = constructProofVote.apply(
						key.params,
						encryptedCapability.encCap(),
						encryptedChoice.encChoice().m(),
						desiredContext,
