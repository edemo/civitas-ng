...
public interface ElGamalCiphertextTestData extends ElgamalMsgTestData,
	...
	ElGamalCiphertext CIPHERTEXT_E = new ElGamalCiphertext(
	...
	ElGamalCiphertext CIPHERTEXT_ENCCAP = new ElGamalCiphertext(
	...
	CivitasBigInteger CIPHERTEXT_EPRIME_B = MESSAGE_VOTE_CAPABILITY_SHARE_ENCODED
			.modMultiply(PUBKEY_E.modPow(FACTOR_EPRIME, BIGINT_P), BIGINT_P);
	ElGamalCiphertext CIPHERTEXT_EPRIME = new ElGamalCiphertext(
			CIPHERTEXT_EPRIME_A, CIPHERTEXT_EPRIME_B);

	ElGamalCiphertext EL_GAMAL_CIPHERTEXT = new ElGamalCiphertext(
	...
	ElGamalCiphertext ENCRYPTED_ZERO_FACTOR = new ElGamalCiphertext(
			ONE, G_EXP_B);
