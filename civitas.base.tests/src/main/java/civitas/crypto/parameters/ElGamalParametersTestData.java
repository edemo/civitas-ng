...
public interface ElGamalParametersTestData extends BasicValuesTestData {
	...
	CivitasBigInteger BIGINT_P = CivitasBigIntegerFactory
			.obtain(Base64.getDecoder().decode(BIGINT_P_BASE64));
	String BIGINT_Q_BASE64 = "ALEbdrNCujCQI0Y4f38yusJkfR1nVqT95/P/H2z28zIH";
	CivitasBigInteger BIGINT_Q = CivitasBigIntegerFactory
			.obtain(Base64.getDecoder().decode(BIGINT_Q_BASE64));
	...
	CivitasBigInteger BIGINT_G = CivitasBigIntegerFactory
			.obtain(Base64.getDecoder().decode(BIGINT_G_BASE64));
	...
	String GENERATOR_OTHER_BASE64 = "";
	CivitasBigInteger GENERATOR_OTHER = CommonUtil
			.asBigint(GENERATOR_OTHER_BASE64);
	CivitasBigInteger GENERATOR_OTHER_POW_R1 = GENERATOR_OTHER.modPow(RANDOMS_1,
			BIGINT_P);
	String GENERATOR_OTHER__POW_R1_BASE64 = CommonUtil
			.fromBigInt(GENERATOR_OTHER_POW_R1);

	CivitasBigInteger GENERATOR_FOR_UNPRIME_P = CommonUtil
			.asBigint("APPuKv0SXyLaZklKlEiPMfeSkEfsEfdN24AXL3NrYBKG");
	CivitasBigInteger GENERATOR_FOR_UNPRIME_Q = CommonUtil
			.asBigint("SlVGgqFXxlg=");

	ElGamalParameters EL_GAMAL_PARAMETERS = new ElGamalParameters(BIGINT_P,
			BIGINT_Q, BIGINT_G);

	ElGamalParameters EL_GAMAL_PARAMETERS_NEGATIVE_P = new ElGamalParameters(
			BIGINT_P.multiply(CivitasBigIntegerFactory.obtain(-1)), BIGINT_Q,
			BIGINT_G);
	ElGamalParameters EL_GAMAL_PARAMETERS_OTHER = new ElGamalParameters(P2, Q2,
			G2);

	ElGamalParameters EL_GAMAL_PARAMETERS_Q_SAME = new ElGamalParameters(
			BIGINT_P_OTHER, BIGINT_Q, BIGINT_G_OTHER);

	ElGamalParameters EL_GAMAL_PARAMETERS_GENERATOR_OTHER = new ElGamalParameters(
	...
	CivitasBigInteger SAFE_P = CivitasBigIntegerFactory
			.obtain(Base64.getDecoder().decode(SAFE_P_BASE64));
	CivitasBigInteger SAFE_Q = CivitasBigIntegerFactory
			.obtain(Base64.getDecoder().decode(SAFE_Q_BASE64));
	CivitasBigInteger SAFE_G = CivitasBigIntegerFactory
			.obtain(Base64.getDecoder().decode(SAFE_G_BASE64));

	PrimePair SAFE_PRIMES = new PrimePair(SAFE_P, SAFE_Q);
	PrimePair SCHNORR_PRIMES = new PrimePair(BIGINT_P, BIGINT_Q);
	ElGamalParameters EL_GAMAL_PARAMETERS_SAFE = new ElGamalParameters(SAFE_P,
			SAFE_Q, SAFE_G);
	CivitasBigInteger PLAINTEXT_WITH_LEGENDRE_MINUS_ONE = CivitasBigIntegerFactory
			.obtain(251121);
	CivitasBigInteger PUBLICIZED_SAFE_LEGENDRE = SAFE_P
			.subtract(PLAINTEXT_WITH_LEGENDRE_MINUS_ONE);
	CivitasBigInteger D_EXP_TWOK_FROMP = BIGINT_D
	...
	Map<CivitasBigInteger, VoteChoice> DECODEMAP = CHOICES.stream()
			.map(x -> new AbstractMap.SimpleEntry<>(CivitasBigIntegerFactory
					.obtain(x.ordinal()).modMultiply(BIGINT_G, BIGINT_P), x))
			.collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue));

	CivitasBigInteger I_BEATS_J_ENCODED = CivitasBigIntegerFactory
			.obtain(VoteChoice.I_BEATS_J.ordinal()).modMultiply(BIGINT_G, BIGINT_P);

	CivitasBigInteger J_BEATS_I_ENCODED = CivitasBigIntegerFactory
			.obtain(VoteChoice.J_BEATS_I.ordinal()).modMultiply(BIGINT_G, BIGINT_P);
	...
	CivitasBigInteger BIGINT_NO_PRIME_OF_GROUPLENGTH = CommonUtil
			.asBigint(BIGINT_NO_PRIME_OF_GROUPLENGTH_BASE64);
	CivitasBigInteger BIGINT_NO_PRIME_OF_KEYLENGTH = CommonUtil
			.asBigint(BIGINT_NO_PRIME_OF_KEYLENGTH_BASE64);

	String EL_GAMAL_PARAMETERS_XML = "<p>" + BIGINT_P_BASE64 + "</p><q>"
			+ BIGINT_Q_BASE64 + "</q><g>" + BIGINT_G_BASE64 + "</g>";
