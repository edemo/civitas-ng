...
//        ps = (ElGamalParametersC) f.generateElGamalParameters(160, 1024);
    ...
    public static ElGamalMsg distDecrypt(Label   ElGamalCiphertext c, ElGamalKeyPairShare[] keys) {
    ...
    static void foo() {
        ...
        VoterSubmission vs1 = new VoterSubmission().civitas$common$VoterSubmission$(  2, verifV1);
        
        StringWriter sb = new StringWriter();
        vs1.toXML(  new PrintWriter(sb));
        String vxml = sb.toString();
        try {
            VoterSubmission vs2 = VoterSubmission.fromXML(  new StringReader(vxml));
...
//        System.err.println("md = dec(cf) = " + md.intValue());
    ...
    private static void decryptionShareTest() {
        ...
        for (int i = 0; i < tellerShares.length; i++) {
            ...
            StringWriter sb = new StringWriter();
            tellerShares[i].toXML(  new PrintWriter(sb));
            String orig = sb.toString();
            try {
                sb = new StringWriter();
                ElGamalKeyPairShare.fromXML(  reader(orig)).toXML(  new PrintWriter(sb));
                test("XML ElGamalKeyPairShare", orig.equals(sb.toString()));
                orig = ((ElGamalKeyShareC)tellerPubShares[i]).toXML();
                test("XML ElGamalKeyShareC", orig.equals(ElGamalKeyShareC.fromXML(  reader(orig)).toXML()));
            }
            catch (IllegalArgumentException e) {
                // TODO Auto-generated catch block
                e.printStackTrace();
        ...
        for (int i = 0; i < tellerShares.length; i++) {
            ...
            try {
                test("XML ElGamalDecryptionShareC", orig.equals(ElGamalDecryptionShareC.fromXML(  reader(orig)).toXML()));
            }
            catch (IllegalArgumentException e) {
                // TODO Auto-generated catch block
                e.printStackTrace();
    ...
    private static void performance() {
        ...
        for (int i = 0; i < ps.length; i++) {
            ...
            for (int j = 0; j < COUNT; j++) {
                f.constructDecryptionShare(    encCap, keyShare);
            }
...
//      ElGamalPublicKeyC.java 
        orig = K.toXML();
        test("XML ElGamalPublicKeyC", orig.equals(ElGamalPublicKeyC.fromXML(  reader(orig)).toXML()));

//      ElGamalPrivateKeyC.java 
        orig = k.toXML();
        test("XML ElGamalPrivateKeyC", orig.equals(ElGamalPrivateKeyC.fromXML(  reader(orig)).toXML()));

        // ElGamalCiphertextC.java 
        orig = c.toXML();
        test("XML ElGamalCiphertextC", orig.equals(ElGamalCiphertextC.fromXML(  reader(orig)).toXML()));

//      ElGamalParametersC.java
        orig = ps.toXML();
        test("XML ElGamalParametersC", orig.equals(ElGamalParametersC.fromXML(  reader(orig)).toXML()));
